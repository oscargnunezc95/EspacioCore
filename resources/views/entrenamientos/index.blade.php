<x-app-layout>
    <x-slot name="header">
        ...
        <!-- 2. Cabecera con Migajas, Título y Botón -->
        <div class="mt-8">
            <x-studio-header 
            ...
            >
                <x-slot name="actions">
                    <button onclick="document.getElementById('monthModal').classList.remove('hidden')" class="bg-zinc-900 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-zinc-800 focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 transition-all duration-200 shadow-sm active:scale-95">
                        + Planificar Mes
                    </button>
                </x-slot>
            </x-studio-header>
    ...
    <div class="py-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse($months as $month)
                ...
                <!-- Tarjeta de Mes -->
                <a href="{{ route('entrenamientos.show', $month->month_id) }}" class="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 group flex flex-col justify-between">
                    <div>
    ...
    <div id="monthModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 bg-zinc-900/60 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-2xl p-6 md:p-8 max-w-md w-full shadow-xl border border-zinc-100 transform transition-all">
            ...
            <div class="flex justify-between items-center mb-6">
                ...
                <button type="button" onclick="document.getElementById('monthModal').classList.add('hidden')" class="text-zinc-400 hover:text-zinc-600 transition-colors">
            ...
            <form action="{{ route('entrenamientos.store') }}" method="POST">
                ...
                <div class="mb-8">
                    ...
                    <div class="space-y-2 max-h-60 overflow-y-auto border border-zinc-200 rounded-xl p-3 bg-zinc-50/50 hide-scrollbar {{ $errors->has('workshops') ? 'border-rose-500 ring-1 ring-rose-500' : '' }}">
                        @forelse($workshops as $w)
                            @php 
                                // Días de la semana, alineados con Carbon (0=Domingo, 6=Sábado)
                            ...
                            <label class="flex items-start gap-3 p-3 bg-white border border-zinc-200 rounded-lg cursor-pointer hover:border-zinc-400 hover:shadow-sm transition-all duration-200">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-zinc-500 text-xs font-medium mt-1">
                                        ...
                                        @elseif(is_array($w->repeat_days) && count($w->repeat_days) > 0)
                                            Se repite los {{ implode(', ', array_map(fn($d) => $dias[$d] ?? '', $w->repeat_days)) }}
                                        @else
                                            <span class="text-rose-500">Sin días asignados</span>
                                        @endif
                                    </span>
                ...
                <div class="flex gap-3 pt-2 border-t border-zinc-100">
                    <button type="button" onclick="document.getElementById('monthModal').classList.add('hidden')" class="w-full font-bold text-zinc-600 bg-zinc-100 hover:bg-zinc-200 py-3 rounded-xl transition-colors duration-200 text-sm">Cancelar</button>
