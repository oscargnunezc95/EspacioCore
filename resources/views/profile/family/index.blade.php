<x-app-layout>
    <div class="py-8 md:py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24" x-data="{
        ...
        <div class="text-center mb-10">
            <div>
                <h1 class="text-3xl md:text-4xl font-black tracking-tight">Mi Familia</h1>
                <p class="mt-3 text-stone-500 font-medium text-base md:text-lg">Gestiona a tus dependientes o revisa las familias a las que perteneces.</p>
            </div>
        </div>
        ...
        <div class="border-b border-stone-500 mb-6 flex gap-6 overflow-x-auto custom-scrollbar">
            <button @click="activeTab = 'mis-dependientes'"
                    :class="activeTab === 'mis-dependientes' ? 'border-red-600 text-stone-500' : 'border-transparent text-stone-400 hover:text-stone-600'"
            ...
            <button @click="activeTab = 'membresias'"
                    :class="activeTab === 'membresias' ? 'border-red-600 text-stone-900' : 'border-transparent text-stone-400 hover:text-stone-600'"
        ...
        <div x-show="activeTab === 'mis-dependientes'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
            <div class="bg-white rounded-2xl md:rounded-3xl shadow-sm border border-stone-200 overflow-hidden">
                <table class="w-full text-left text-sm">
                    <thead class="bg-stone-50 border-b border-stone-200 text-stone-400 font-black uppercase text-[10px] tracking-widest">
                        <tr>
                    ...
                    <tbody class="divide-y divide-stone-100">
                        @forelse($dependents as $dependent)
                            <tr class="hover:bg-stone-50/50 transition-colors group {{ $dependent->status === 'pending' ? 'bg-amber-50/30' : '' }}">
                                <td class="px-6 py-4 font-bold text-stone-900">
                                    <div class="flex items-center gap-2">
                                        {{ $dependent->first_name }} {{ $dependent->last_name }}
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-stone-500 font-medium">{{ $dependent->national_id ?? 'No registrado' }}</td>
                                <td class="px-6 py-4">
                                    <span class="bg-stone-100 text-stone-700 px-2.5 py-1 rounded-lg font-bold text-[11px] uppercase tracking-wider">{{ $dependent->relationship }}</span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        @if($dependent->status === 'active')
                                            <button @click="editMode = true; actionUrl = '/profile/familia/{{ $dependent->id }}'; formData = { first_name: '{{ $dependent->first_name }}', last_name: '{{ $dependent->last_name }}', country_id: '{{ $dependent->country_id ?? '' }}', national_id: '{{ $dependent->national_id }}', relationship: '{{ $dependent->relationship }}' }; openModal = true" class="p-2 text-stone-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all">
                                        ...
                                        <form action="{{ route('profile.family.destroy', $dependent->id) }}" method="POST" onsubmit="return confirm('¿Eliminar este perfil?')">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="p-2 text-stone-400 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-stone-400">No administras familiares.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="flex justify-end py-8">
                <button @click="editMode = false; actionUrl = '{{ route('profile.family.store') }}'; conflictType = ''; formData = { first_name: '', last_name: '', country_id: '1', national_id: '', relationship: 'Hijo/a' }; openModal = true" class="bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition-all duration-300 active:scale-95 flex items-center gap-2">
                    ...
                    <tbody class="divide-y divide-stone-100">
                        @forelse($memberships as $membership)
                            <tr class="hover:bg-stone-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <p class="font-bold text-stone-900">{{ $membership->user->name }}</p>
                                    <p class="text-[11px] text-stone-500 font-medium">{{ $membership->user->email }}</p>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="bg-stone-100 text-stone-700 px-2.5 py-1 rounded-lg font-bold text-[11px] uppercase tracking-wider">{{ $membership->relationship }}</span>
                                </td>
                                <td class="px-6 py-4">
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center text-stone-400">
                                    <svg class="w-12 h-12 mx-auto text-stone-200 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
        ...
        {{-- ======================================================= --}}
        <div x-show="openModal && !conflictType" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm" x-cloak x-transition>
            <div @click.away="openModal = false" class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                <div class="p-6 border-b border-stone-100 flex justify-between items-center">
                    <h3 class="text-xl font-black text-stone-900" x-text="editMode ? 'Modificar Familiar' : 'Nuevo Familiar'"></h3>
                    <button @click="openModal = false" class="text-stone-400 hover:text-stone-600"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
                </div>
                <form :action="actionUrl" method="POST" class="p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-[11px] font-black text-stone-400 uppercase tracking-widest mb-1">Nombre</label>
                            <input type="text" name="first_name" x-model="formData.first_name" required class="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-[11px] font-black text-stone-400 uppercase tracking-widest mb-1">Apellido</label>
                            <input type="text" name="last_name" x-model="formData.last_name" class="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-stone-400 uppercase tracking-widest mb-1">País del Documento</label>
                        <select name="country_id" x-model="formData.country_id" required class="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all bg-white">
                    ...
                    <div>
                        <label class="block text-[11px] font-black text-stone-400 uppercase tracking-widest mb-1">RUT / Documento</label>
                        <input type="text" name="national_id" x-model="formData.national_id" placeholder="Ej: 12345678-9" required class="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-[11px] font-black text-stone-400 uppercase tracking-widest mb-1">Parentesco</label>
                        <select name="relationship" x-model="formData.relationship" class="w-full rounded-xl border border-stone-200 px-3 py-2 text-sm focus:ring-2 focus:ring-red-500 outline-none transition-all bg-white">
                    ...
                    <button type="submit" class="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition-all duration-300 active:scale-95 mt-4">
        ...
        <div x-show="conflictType === 'transfer'" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <p class="text-sm text-stone-700"><strong x-text="conflictData?.dependent_name || 'Esta persona'"></strong> ya figura como familiar de <strong class="text-red-600" x-text="conflictData?.owner_name || 'otro usuario'"></strong>.</p>
                    ...
                    <button @click="conflictType = ''; openModal = true" class="w-full bg-stone-100 text-stone-600 font-bold py-2.5 rounded-xl hover:bg-stone-200">Cancelar</button>
                </div>
            </div>
        </div>

        {{-- Conflicto 2: Usuario Global --}}
        <div x-show="conflictType === 'link'" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm" x-cloak>
            <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
                <div class="p-6 border-b border-red-100 bg-red-50">
                    <h3 class="text-lg font-black text-red-800 flex items-center gap-2"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Usuario Independiente</h3>
                </div>
                <div class="p-6 space-y-4">
                    <p class="text-sm text-stone-700">El documento ingresado pertenece a <strong class="text-red-600" x-text="conflictData?.user_name || 'un usuario'"></strong>, que ya tiene una cuenta propia.</p>
                    <form action="{{ route('profile.family.store') }}" method="POST" class="space-y-3">
                        @csrf
                        ...
                        <input type="hidden" name="national_id" :value="formData.national_id">
                        <input type="hidden" name="relationship" :value="formData.relationship">
                        <button type="submit" class="w-full bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition-all duration-300 active:scale-95">Enviar solicitud de autorización</button>
                    </form>
                    <button @click="conflictType = ''; openModal = true" class="w-full bg-stone-100 text-stone-600 font-bold py-2.5 rounded-xl hover:bg-stone-200">Cancelar</button>
                </div>
            </div>
