<x-app-layout>
    <div class="py-8 md:py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
        ...
        <div class="text-center mb-8 md:mb-12">
            <h1 class="text-3xl md:text-4xl font-black tracking-tight">Pasar Lista</h1>
            <p class="mt-2 md:mt-3 text-stone-500 text-base md:text-lg">Gestiona la asistencia de tus estudiantes.</p>
        </div>

        {{-- Cabecera Unificada (Card Header) --}}
        <div class="mt-2 mb-8 p-1">
            {{-- Breadcrumbs con navegación funcional --}}
            <nav class="flex text-xs font-bold text-stone-500 mb-3 gap-2 items-center">
                <a href="{{ route('global.classes.teacher') }}" class="hover:text-stone-900 transition-colors">Mi Agenda</a>
                <span>/</span>
                <span class="text-stone-900">Lista de Asistencia</span>
            </nav>
        ...
        <div class="bg-white rounded-2xl md:rounded-3xl shadow-lg border border-stone-200 overflow-hidden mb-12 {{ $session->is_cancelled ? 'opacity-75 grayscale-[20%]' : '' }}">
            ...
            <div class="relative min-h-[200px] md:min-h-[240px] w-full bg-zinc-900 flex flex-col justify-end">
                ...
                <div class="relative z-10 p-5 sm:p-6 md:p-8 w-full mt-auto">
                    ...
                    <div class="flex items-center gap-2 mb-3">
                        <img src="{{ $studioAvatar }}" alt="Studio" class="w-6 h-6 md:w-8 md:h-8 rounded-md md:rounded-lg object-cover ring-2 ring-white/30 shadow-sm bg-stone-900">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 md:gap-4">
                        <div class="flex items-center gap-2 text-stone-800 bg-white/95 px-3 md:px-4 py-2 md:py-2.5 rounded-xl border border-stone-100/70 inline-flex shadow-xl backdrop-blur-md">
                            <svg class="w-4 h-4 md:w-5 md:h-5 text-red-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            <span class="capitalize text-xs md:text-sm font-bold">{{ $parsedMonth->translatedFormat('l d \d\e F') }}</span>
                            <span class="text-stone-300 mx-1">|</span>
                            <span class="font-black text-stone-900 text-xs md:text-sm">{{ \Carbon\Carbon::parse($session->start_time)->format('H:i') }} hrs</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="flex flex-row items-center justify-between gap-3 md:gap-4 w-full">
                    ...
                    <div class="flex-1 relative min-w-0">
                        ...
                        <input type="text" id="searchStudent" onkeyup="filterStudents()" placeholder="Buscar alumna/o..." 
                               class="w-full pl-9 pr-3 py-2 md:py-2.5 rounded-xl border border-stone-300 bg-white text-stone-900 text-sm focus:ring-2 focus:ring-stone-900 focus:border-stone-900 transition-all outline-none shadow-sm {{ $session->is_cancelled ? 'pointer-events-none bg-stone-100' : '' }}">
                    </div>
            ...
            <ul class="divide-y divide-stone-100" id="studentsList">
                @forelse($students as $student)
                    ...
                    @endphp
                    <li class="student-item p-4 md:p-6 flex items-center gap-4 transition-colors hover:bg-stone-50/80 {{ $session->is_cancelled ? 'pointer-events-none' : '' }}">
                        
                        {{-- Switch de Asistencia --}}
                        <button onclick="toggleAttendance('{{ $toggleUrl }}', this)" 
                                class="relative inline-flex h-7 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-stone-900 focus:ring-offset-2 {{ $isPresent ? 'bg-emerald-500' : 'bg-stone-200' }}" 
                        ...
                        <div class="flex items-center gap-3 min-w-0 flex-1">
                            <img src="{{ $studentAvatar }}" class="w-9 h-9 rounded-full border border-red-100 shadow-sm shrink-0" alt="Avatar">
                            <div class="min-w-0 flex-1">
                                <p class="student-name font-bold text-stone-900 text-sm md:text-base leading-tight truncate">{{ $student->name }}</p>
                                <p class="text-[10px] md:text-[11px] font-bold text-stone-400 mt-0.5 uppercase tracking-wider truncate">
                ...
                @empty
                    <li class="p-10 text-center text-sm font-medium text-stone-500 flex flex-col items-center justify-center">
                        <div class="w-12 h-12 rounded-full bg-stone-100 flex items-center justify-center mb-3">
                            <svg class="w-6 h-6 text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>
                        </div>
...
<script>
    ...
    function toggleAttendance(url, btn) {
        ...
        .catch(error => {
            ...
            btn.classList.replace(wasPresent ? 'bg-emerald-500' : 'bg-stone-200', wasPresent ? 'bg-stone-200' : 'bg-emerald-500');
