...
<div class="py-8 md:py-8 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
        ...
        <div class="text-center mb-12">
            ...
            <p class="mt-3 text-stone-500 font-medium">
        ...
        <div class="bg-white rounded-2xl border border-stone-200/60 shadow-sm overflow-hidden"
            ...
            <div class="grid grid-cols-2 bg-stone-100/80 p-1.5 m-4 rounded-xl">
                <button type="button"
...
<!--                 <button type="button"
            ...
            <form action="{{ route('support.store') }}" method="POST" class="px-6 pb-6 space-y-5">
                ...
                {{-- Nombre --}}
                <div>
                    <label for="name" class="block text-sm font-bold text-stone-700 mb-1.5">
                        Nombre completo
                    </label>
                    <input id="name" name="name" type="text" value="{{ old('name') }}" required autocomplete="name"
                           class="block w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-900 placeholder-stone-400 focus:outline-none focus:bg-white focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200 sm:text-sm"
                ...
                <div x-show="isInquiry" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0">
                    <label for="message" class="block text-sm font-bold text-stone-700 mb-1.5">
                        Tu mensaje
                    </label>
                    <textarea id="message" name="message" rows="4"
                              :disabled="!isInquiry"
                              class="block w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-900 placeholder-stone-400 focus:outline-none focus:bg-white focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200 sm:text-sm resize-none disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                <div x-show="isDemo" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" class="space-y-5">
                    {{-- Fecha --}}
                    <div>
                        <label for="meeting_date" class="block text-sm font-bold text-stone-700 mb-1.5">
                            Fecha de la reunión
                        </label>
                        <input id="meeting_date" name="meeting_date" type="date"
                               ...
                               class="block w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-900 focus:outline-none focus:bg-white focus:border-red-500 focus:ring-2 focus:ring-red-500/20 transition-all duration-200 sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed">
                        <p class="mt-1.5 text-xs text-stone-400 font-medium">Selecciona a partir de mañana.</p>
                    </div>

                    {{-- Hora --}}
                    <div>
                        <label for="meeting_time" class="block text-sm font-bold text-stone-700 mb-1.5">
                            Hora de la reunión
                        </label>
                        ...
                        <p class="mt-1.5 text-xs text-stone-400 font-medium">Horario de Chile continental (GMT-4).</p>
                    </div>

                    {{-- Mensaje opcional para demo --}}
                    <div>
                        <label for="demo_message" class="block text-sm font-bold text-stone-700 mb-1.5">
                            Mensaje <span class="text-stone-400 font-normal">(opcional)</span>
                        </label>
                ...
                <div class="pt-2">
                    <button type="submit"
                            ...
                            :class="isInquiry
                                ? 'bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition-all duration-300 active:scale-95'
                                : 'bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-4 rounded-xl shadow-sm transition-all duration-300 active:scale-95'"
        ...
        {{-- Footer sutil --}}
        <p class="mt-8 text-center text-xs text-stone-400 font-medium">
            También puedes escribirnos directamente a <a href="mailto:oscar@estadoprisma.test" class="text-red-600 hover:text-red-500 transition-colors duration-200">oscar@estadoprisma.test</a>
        </p>
