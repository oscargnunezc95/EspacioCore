    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('admin.studios.index') }}"
               class="inline-flex items-center gap-1.5 rounded-xl border border-stone-200 bg-white px-3 py-1.5 text-sm font-medium text-stone-600 shadow-sm transition-all duration-200 hover:bg-stone-50 hover:border-stone-300 focus:ring-2 focus:ring-stone-300 focus:ring-offset-2 focus:outline-none">
            ...
            <div class="rounded-2xl border border-stone-200 bg-white p-5">
                <form method="GET" action="{{ route('admin.studios.audit', $studio->id) }}" class="flex items-end gap-4">
                    <div class="flex flex-col gap-1.5">
                        <label for="month" class="text-xs font-semibold uppercase tracking-wider text-stone-500">
                            Mes a Auditar
                        </label>
                        <input type="month"
                               id="month"
                               name="month"
                               value="{{ $selectedMonth }}"
                               class="w-56 rounded-xl border border-stone-300 px-3.5 py-2 text-sm text-stone-900 shadow-sm transition-all duration-200 focus:border-red-500 focus:ring-2 focus:ring-red-200 focus:outline-none">
                    </div>

                    <button type="submit"
                            class="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-bold py-2.5 px-4 shadow-sm transition-all duration-300 active:scale-95 focus:ring-2 focus:ring-red-300 focus:ring-offset-2 focus:outline-none text-sm">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="overflow-x-auto custom-scrollbar">
                    <table class="min-w-full divide-y divide-stone-200 text-sm">
                        <thead class="bg-stone-50">
                            <tr>
                                <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-stone-500">
                                    Fecha
                                </th>
                                <th scope="col" class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-stone-500">
                                    Transacción MP
                                ...
                                <th scope="col" class="px-6 py-3.5 text-right text-xs font-semibold uppercase tracking-wider text-stone-500">
                                    Total Bruto
                                </th>
                                <th scope="col" class="px-6 py-3.5 text-right text-xs font-semibold uppercase tracking-wider text-stone-500">
                                    Nuestra Comisión
                        ...
                        <tbody class="divide-y divide-stone-100">
                            @forelse ($payments as $payment)
                                <tr class="transition-colors duration-200 hover:bg-stone-50/50">
                                    ...
                                    {{-- ID Transacción MP --}}
                                    <td class="whitespace-nowrap px-6 py-4 font-mono text-xs text-stone-500">
                                    ...
                                    {{-- Nuestra Comisión --}}
                                    <td class="whitespace-nowrap px-6 py-4 text-right font-medium text-red-600">
